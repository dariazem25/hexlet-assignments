...
class AppTest {
    ...
    void testGetDirectorySize() throws Exception {
        CompletableFuture<Long> result = App.getDirectorySize("src/test/resources/dir/nested");
        var actual = result.get();
        Assertions.assertEquals(1, actual);
    }

    @Test
    void testGetNonExistedDirectorySize() throws Exception {
        String result = tapSystemOut(() -> {
            App.getDirectorySize("src/test/resources/dir/nested2").get();
        });
        assertThat(result.trim()).contains("Directory does not exist");
    }
    // END
}
